...
namespace windmill {
void Windmill::drawBboxes(const cv::Mat &bgr,
...
void Windmill::cvProcess(const cv::Mat &image) {
  ...
  if (red_)
    cv::inRange(
        hsv, cv::Scalar(red_lower_hsv_h_, red_lower_hsv_s_, red_lower_hsv_v_),
        cv::Scalar(red_upper_hsv_h_, red_upper_hsv_s_, red_upper_hsv_v_),
        threshold);
  else
    cv::inRange(
        hsv,
        cv::Scalar(blue_lower_hsv_h_, blue_lower_hsv_s_, blue_lower_hsv_v_),
        cv::Scalar(blue_upper_hsv_h_, blue_upper_hsv_s_, blue_upper_hsv_v_),
        threshold);
  cv::Mat kernel = cv::getStructuringElement(
      cv::MORPH_RECT, cv::Size(1 + 2 * morph_size_, 1 + 2 * morph_size_),
      cv::Point(-1, -1));
  cv::morphologyEx(threshold, threshold, morph_type_, kernel, cv::Point(-1, -1),
                   morph_iterations_);

  binary_publisher_.publish(
      cv_bridge::CvImage(std_msgs::Header(), "mono8", threshold).toImageMsg());
  ...
  for (auto &contour : contours) {
    ...
    if (cv::matchShapes(contour, r_contour_, cv::CONTOURS_MATCH_I2, 0) <=
            hull_bias_ &&
        area_judge) {
...
void Windmill::threading() {
  std::thread thread_1(
      std::bind(&Windmill::modelProcess, this, std::ref(cv_image_->image)));
  std::thread thread_2(
      std::bind(&Windmill::cvProcess, this, std::ref(cv_image_->image)));
  thread_1.join();
  ...
  if (!hull_vec_.empty() && !box_result_vec_.empty()) {
    ...
    for (const auto &contour : hull_vec_) {
      ...
      double ratio =
          getL2Distance(tmp_point1, tmp_point2) / getL2Distance(tmp_point1, r);
    ...
    std::sort(
        radian_vec.begin(), radian_vec.end(),
        [](const auto &v1, const auto &v2) { return v1.second < v2.second; });
    auto r = radian_vec[0].first;
    cv::circle(cv_image_->image, r, 3, cv::Scalar(0, 255, 0), cv::FILLED);
    r_array[0] = r.x * 1440 / image_size_;
    r_array[1] = r.y * 1080 / image_size_;
    poly_array[0] =
        static_cast<int32_t>(box_result_vec_[0].x1) * 1440 / image_size_;
    poly_array[1] =
        static_cast<int32_t>(box_result_vec_[0].y1) * 1080 / image_size_;
    poly_array[2] =
        static_cast<int32_t>(box_result_vec_[0].x4) * 1440 / image_size_;
    poly_array[3] =
        static_cast<int32_t>(box_result_vec_[0].y4) * 1080 / image_size_;
    poly_array[4] =
        static_cast<int32_t>(box_result_vec_[0].x3) * 1440 / image_size_;
    poly_array[5] =
        static_cast<int32_t>(box_result_vec_[0].y3) * 1080 / image_size_;
    poly_array[6] =
        static_cast<int32_t>(box_result_vec_[0].x2) * 1440 / image_size_;
    poly_array[7] =
