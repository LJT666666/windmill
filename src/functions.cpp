...
void Windmill::onInit() {
  ...
  img_subscriber_ = nh_.subscribe("/hk_camera/camera/image_raw", 1,
                                  &Windmill::receiveFromCam, this);
  this->status_subscriber_ = nh_.advertiseService(
      "/buff_status_switch", &Windmill::receiveFromStatus, this);
  //        img_subscriber_ = nh_.subscribe("/hk_camera/image_raw", 1,
  //        &Windmill::receiveFromCam, this);
  //    img_subscriber_= nh_.subscribe("/image_rect", 1,
  //    &Windmill::receiveFromCam,this);
  ...
  point_publisher_ =
      nh_.advertise<rm_msgs::TargetDetectionArray>("/prediction", 1);
  ...
  std::sort(contours.begin(), contours.end(),
            [](const std::vector<cv::Point> &vec1,
               const std::vector<cv::Point> &vec2) {
...
void Windmill::resizeUniform(const cv::Mat &src, cv::Mat &dst,
